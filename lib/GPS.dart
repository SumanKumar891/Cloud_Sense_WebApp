...
class _MapPageState extends State<MapPage> {
  ...
  final double displacementThreshold = 500.0;
        ...
        for (var device in latestDevices.values) {
          ...
          if (previousPositions.containsKey(deviceId)) {
            ...
            if (dist >= displacementThreshold) {
              ...
              print(
                  'Device $deviceId moved ${dist.toStringAsFixed(2)}m (>500m), setting to red');
            } else {
              hasMoved = prevData['has_moved'] ?? false;
              print('Device $deviceId stationary (<500m), retaining color');
            }
          } else {
            hasMoved = false;
            initialMovedTimestamp = currentTimestamp;
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Status: ${hasMoved ? "Moved (>500m)" : "Stationary (<500m or >10 min)"}',
                    style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.bold,
